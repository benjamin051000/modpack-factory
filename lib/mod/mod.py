...
class Mod:
    ...
    def from_batched(
        ...
        pass  # Only for treesitter context so it doesn't include the block below
                ...
                for files in version["files"]:
                    ...
                    for dependency in constraint_for_this_file.depends:
                        ...
                        def version_is_candidate(
                            dep_version_json: dict, dependency: FabricVersionRange
                        ) -> bool:
                            # Compare with `file` so we use the fabric.mod.json version
                            # NOTE actually, looks like constraint_for_this_file has
                            # all the info we need (?)
                            # Get the constraint_for_this_file.version for dep_version,
                            # and then use that to see?
                            dep_version = (
                            ...
                            foo = dep_version in dependency
                            # print(f"{'keeping' if foo else 'skipping'} {dep_version}")

                            # We may need to grab the version listed in fabric.mod.json,
                            # not the one on modrinth (they are sometimes different.)
                            # For example, one from sodium is mc1.16.3-0.1.0. The actual
                            # version is 0.1.0 I guess. Let's hope there's a clear and
                            # consistent convention... but I'm not holding my breath.
